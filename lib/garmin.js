...
export const endpoints = {
    ...
    // Planlegging av en økt på en dato i treningskalenderen. Biblioteket har
    // ingen scheduleWorkout(), så dette går som rått POST via gc.client.post
    // med kroppen { date: 'YYYY-MM-DD' }; svaret inneholder workoutScheduleId.
    //
    // Sletting av selve økta fjerner OGSÅ kalenderoppføringen (kaskade), så en
    // ny push av samme dato erstatter i stedet for å duplisere — det er dét som
    // gjør push-plan.js idempotent uten en egen «avplanlegg»-sti.
    // Verifisert 01.09.2026.
    scheduleWorkout: (workoutId) => [`${API}/workout-service/schedule/${workoutId}`],

    // Treningskalenderen for én måned. MERK: måneden er 0-indeksert — oktober
    // er month/9. Verifisert 01.09.2026.
    calendar: (year, month0) => [`${API}/calendar-service/year/${year}/month/${month0}`],
